...
public class MainActivity extends AppCompatActivity {
  ...
  private static final List<String> resource = Arrays.asList(
    "final.zip", "units.txt", "ctc.dla", "decoder.dla", "encoder.dla", "embedding_pos.bin","embedding_token.bin"
  );
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    // adb shell pm grant com.mobvoi.wenet android.permission.RECORD_AUDIO
...
/*    try {
    ...
    //Recognize.init("/data/local/tmp/resource"/*getFilesDir().getPath()*/);
  ...
  private short[] getChannelData(short[] data, int channel_index) {
    if (data == null || data.length <= 0) {
      return null;
    }
    short[] result = new short[data.length / 2];
    for (int index = 0; index < data.length / 2;index++) {
      result[index] =  data[index*2 + channel_index];
    }
    return result;
  }
      ...
      while (startRecord) {
        ...
        try {
          if (AudioRecord.ERROR_INVALID_OPERATION != read) {
            ...
            //bufferQueue.put(getChannelData(buffer, 0));
          }
...
//    Process.setThreadPriority(Process.THREAD_PRIORITY_AUDIO);
//
//    // 0.创建音频文件，音频文件名称为当前时间戳.pcm  单通道 16k  --------
//    String fileName = "audio_" + System.currentTimeMillis() + ".pcm";
//    File audioFile = new File("/data/local/tmp/resource", fileName);
...
//      if (read > 0) {
//        // 将short数组转换为byte数组
//        byte[] byteBuffer = new byte[read * 2]; // 每个short占2个字节
